...
use crate::artifacts::semantic_hash_v1::{
    semantic_bytes_for_contract, semantic_bytes_for_service_profile, semantic_bytes_for_state,
    semantic_bytes_for_survey, semantic_bytes_for_validation_report,
    semantic_content_json_for_contract, semantic_content_json_for_service_profile,
    semantic_content_json_for_state, semantic_content_json_for_survey,
    semantic_content_json_for_validation_report, semantic_hash_hex_for_contract,
        ...
        HOST_SURVEY_SCHEMA_ID => {
            let artifact: HostSurveyV1 = serde_json::from_value(raw).map_err(|error| {
                ArtifactRecordError::new(
                    ArtifactRecordErrorCode::ArtifactDecodeInvalid,
                    "artifact_load",
                    format!("failed to decode host-survey.v1 artifact: {error}"),
                )
            })?;
        ...
        HOST_CONTRACT_SCHEMA_ID => {
            let artifact: HostContractV1 = serde_json::from_value(raw).map_err(|error| {
                ArtifactRecordError::new(
                    ArtifactRecordErrorCode::ArtifactDecodeInvalid,
                    "artifact_load",
                    format!("failed to decode host-contract.v1 artifact: {error}"),
                )
            })?;
        ...
        SERVICE_PROFILE_SCHEMA_ID => {
            let artifact: ServiceProfileV1 = serde_json::from_value(raw).map_err(|error| {
                ArtifactRecordError::new(
                    ArtifactRecordErrorCode::ArtifactDecodeInvalid,
                    "artifact_load",
                    format!("failed to decode service-profile.v1 artifact: {error}"),
                )
            })?;
        ...
        HOST_STATE_SCHEMA_ID => {
            let artifact: HostStateV1 = serde_json::from_value(raw).map_err(|error| {
                ArtifactRecordError::new(
                    ArtifactRecordErrorCode::ArtifactDecodeInvalid,
                    "artifact_load",
                    format!("failed to decode host-state.v1 artifact: {error}"),
                )
            })?;
        ...
        VALIDATION_REPORT_SCHEMA_ID => {
            let artifact: ValidationReportV1 = serde_json::from_value(raw).map_err(|error| {
                ArtifactRecordError::new(
                    ArtifactRecordErrorCode::ArtifactDecodeInvalid,
                    "artifact_load",
                    format!("failed to decode validation-report.v1 artifact: {error}"),
                )
            })?;
